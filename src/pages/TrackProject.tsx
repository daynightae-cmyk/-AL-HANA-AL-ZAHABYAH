...
export const TrackProjectPage: React.FC = () => {
  ...
  useEffect(() => {
    if (codeParam) {
      ...
      setProject(getProjectByCode(codeParam) || null);
  ...
  const stages = [
    ...
    },
    {
      ...
      desc_ar: 'توقيع العقد الرسمي وتوريد الخامات المعتمدة للموقع.',
      desc_en: 'Agreement signed and approved materials delivered to site.'
    },
    {
      num: 4,
      title_ar: 'تنفيذ أعمال الدهان والديكور',
      title_en: 'Active Site Execution',
      desc_ar: 'تنفيذ الدهانات والجبس بورد والديكور حسب الخطة.',
      desc_en: 'Painting, gypsum, and decoration works are being executed.'
    },
    {
      num: 5,
      title_ar: 'التنظيف النهائي وفحص الجودة',
      title_en: 'QC & Handover Audit',
      desc_ar: 'التنظيف النهائي وفحص الجودة وتسليم المشروع.',
      desc_en: 'Final cleaning, quality check, and project handover.'
    }
  ];
      ...
      <section className="relative py-12 px-4 text-center border-b border-amber-500/10 bg-gradient-to-b from-neutral-950 to-neutral-900">
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          <span className="text-xs font-mono text-amber-500 font-bold uppercase tracking-widest px-3 py-1 rounded bg-amber-500/5 border border-amber-500/15">
            {language === 'ar' ? 'تتبع المشاريع في الإمارات' : 'Live Construction Timeline'}
          </span>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-12 w-full flex-1">
        <div className="p-6 rounded-xl bg-neutral-900 border border-neutral-850 shadow-md max-w-2xl mx-auto mb-10 text-center">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            ...
            <button type="submit" className="px-6 py-3 rounded-lg bg-amber-500 hover:bg-amber-600 text-neutral-950 text-xs font-extrabold uppercase font-mono tracking-wider cursor-pointer">
          ...
          <div className="mt-4 pt-4 border-t border-neutral-850 flex items-center justify-center gap-3 flex-wrap text-xs text-neutral-500">
            ...
            {['AHZ-2026-0001', 'AHZ-2026-0002'].map((code) => (
              <button key={code} onClick={() => { setInputCode(code); setSearchParams({ code }); }} className="px-2 py-1 rounded bg-neutral-950 text-amber-500 hover:text-amber-400 border border-neutral-800 font-mono text-[10px]">
                {code}
              </button>
            ))}
          </div>
        ...
        {searchAttempted && (
          project ? (
            <div className="space-y-8 animate-fade-in text-start">
              <div className="p-6 rounded-xl bg-neutral-900 border border-neutral-850 shadow-md grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-amber-500 block mt-1">{project.service_type.replace('-', ' ').toUpperCase()}</span>
                  <div className="flex items-center gap-1.5 text-xs text-neutral-400 mt-1">
                      ...
                      <div key={stg.num} className="flex items-start gap-4 relative pl-8 rtl:pl-0 rtl:pr-8">
                        <div className="absolute left-0 rtl:left-auto rtl:right-0 top-1 z-10 flex items-center justify-center">
                          {isCompleted ? (
                            ...
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-extrabold text-xs ${isActive ? 'bg-amber-500 text-neutral-950 animate-pulse' : 'bg-neutral-950 border-2 border-neutral-850 text-neutral-600'}`}>
                              {stg.num}
                            </div>
                          )}
                        </div>
                        <div className={`p-4 rounded-lg flex-1 border ${isActive ? 'bg-amber-500/5 border-amber-500/20' : isCompleted ? 'bg-neutral-900/60 border-neutral-850' : 'bg-neutral-950/30 border-neutral-950/30'}`}>
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                            <span className={`text-sm font-bold ${isActive ? 'text-amber-400' : isCompleted ? 'text-white' : 'text-neutral-500'}`}>{language === 'ar' ? stg.title_ar : stg.title_en}</span>
                            {isActive && <span className="text-[9px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 font-mono font-bold uppercase tracking-wide inline-block max-w-[100px] text-center">{language === 'ar' ? 'جاري العمل' : 'In Progress'}</span>}
                          </div>
                          <p className={`text-xs mt-1.5 leading-relaxed ${isActive ? 'text-neutral-300' : isCompleted ? 'text-neutral-400' : 'text-neutral-600'}`}>{language === 'ar' ? stg.desc_ar : stg.desc_en}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div className="md:col-span-5 p-6 rounded-xl bg-neutral-900 border border-neutral-850 flex flex-col justify-between space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-neutral-950 border border-amber-500/25 flex items-center justify-center text-amber-400"><UserCheck className="w-6 h-6" /></div>
                    <div className="text-start">
                      <span className="text-[10px] uppercase font-mono text-neutral-500">Assigned Engineer</span>
                      <h4 className="text-sm font-bold text-white block mt-0.5">{project.supervisor_name}</h4>
                    </div>
                  </div>
                  <div className="space-y-2 pt-2 border-t border-neutral-800">
                    <a href={`tel:${COMPANY.phone}`} className="flex items-center justify-center gap-2 w-full py-2.5 rounded bg-neutral-950 hover:bg-neutral-900 border border-neutral-800 hover:border-amber-500/25 text-xs text-neutral-300 font-bold transition-all font-mono">
                      <Phone className="w-4 h-4 text-amber-500" />
                      <span>{t.phone_label}: {COMPANY.phoneDisplay}</span>
                    </a>
                    <a href={COMPANY.whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-2.5 rounded bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/25 text-xs font-bold transition-all">
                ...
                <div className="md:col-span-7 p-6 rounded-xl bg-neutral-900 border border-neutral-850 text-start space-y-3">
                  ...
                  <div className="p-4 bg-neutral-950 border border-neutral-850 rounded-lg">
                    <p className="text-xs sm:text-sm text-neutral-300 leading-relaxed font-sans italic">&quot;{language === 'ar' ? project.latest_remark_ar : project.latest_remark_en}&quot;</p>
                  </div>
            ...
            <div id="project-not-found-banner" className="p-8 sm:p-12 rounded-xl bg-neutral-900 border border-neutral-850 shadow-md text-center space-y-4 max-w-xl mx-auto">
              <AlertCircle className="w-12 h-12 text-amber-500 mx-auto" />
              <h3 className="text-base font-bold text-white">{language === 'ar' ? 'رمز تتبع غير صحيح' : 'Invalid Tracking Code'}</h3>
              <p className="text-xs text-neutral-400 leading-relaxed">
                {language === 'ar'
                  ? 'عذراً، لم نتمكن من العثور على أي مشروع مرتبط بالرمز المدخل. يرجى مراجعة الرمز والتأكد من تطابقه.'
                  : 'We could not map the specified reference to any active decoration log. Make sure capitalization and dashes match.'}
              </p>
              <Link to="/request-quote" className="inline-flex px-5 py-2.5 rounded bg-amber-500 text-neutral-950 text-xs font-bold">{t.btn_request_quote}</Link>
            </div>
          )
        )}
      </section>
